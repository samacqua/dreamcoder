...
class TokenRecurrentFeatureExtractor(RecurrentFeatureExtractor):
    ...
    def __init__(
        ...
        super(TokenRecurrentFeatureExtractor, self).__init__(
            lexicon=self.build_lexicon(lexicon),
            H=H,
            tasks=tasks,
            bidirectional=True,
            cuda=cuda,
    ...
    def tokenize(self, task):
        ...
        if task_name in self.tokenized_tasks:
            return self.tokenized_tasks[task_name]
        elif task_name in self.tokenized_helmholtz:
            return self.tokenized_helmholtz[task_name]

        elif (
            type(task) != str
            and task.nearest_name is not None
            and task.nearest_name in self.tokenized_tasks
        ):
            # Returns tokens for the nearest task
            return self.tokenized_tasks[task.nearest_name]
        elif task_name not in self.language_data:
            print("Not found! ", task.name)
            if task_name not in self.language_data:
                self.language_data[task_name] = [self.UNK]
            else:
                use_task_name = task_name
        tokens = []
        for sentence in self.language_data[use_task_name]:
            ...
            if self.canonicalize_numbers:
                sentence_tokens = [
                    self.canonicalize_number_token(t) for t in sentence_tokens
                ]
            tokens.append(
                sentence_tokens
            )  # Feature extractor examples are usually inputs and outputs
        # We pass in all of the tokens as separate 'arguments'
        self.tokenized_tasks[use_task_name] = [[self.add_unk(tokens), []]]
        return self.tokenized_tasks[
            use_task_name
        ]  # Feature extractor examples are usually lists of (xs, y) sets.

    def add_unk(self, sentences):
        return [
            [t if t in self.symbolToIndex else "UNK" for t in sentence]
    ...
    def tokenize_for_smt(self, task):
        ...
        for (sentences, _) in self.tokenize(task):
            for tokens in sentences:
                ...
                numeric_tokenized.append(
                    [self.symbolToIndex[token] for token in tokens]
                )
    ...
    def update_with_tokenized_helmholtz(self, helmholtz_frontiers, grammar):
        eprint(
            f"[TokenRecurrentFeatureExtractor] Received n={len(helmholtz_frontiers)} Helmholtz frontiers; resetting Helmholtz tokens."
        )
        task_to_tokens = translate_frontiers_to_nl(
            helmholtz_frontiers,
            grammar,
            self.smt_translator_info,
            self.n_best,
            verbose=False,
        )
        self.tokenized_helmholtz = {
            f.task.name: [[self.add_unk(task_to_tokens[f.task]), []]]
...
class NgramFeaturizer(nn.Module):
    ...
    def fit_language_features(self, sorted_keys, language_data):
        ...
        eprint(
            "Fitting language data with n-gram features: {} features from {} tasks".format(
                n_features, len(language_data)
            )
        )
    ...
    def featuresOfTask(self, t):
        ...
        if t not in self.featurized_language_data:
            return None
...
class LogLinearBigramTransitionParser(nn.Module):
    ...
    def __init__(
        ...
        self.language_featurizer = language_feature_extractor(
            tasks, testingTasks, cuda, language_data
        )
        ...
        self.log_linear_bigram_network = ContextualGrammarNetwork(
            inputDimensionality=self.language_featurizer.outputDimensionality,
            grammar=self.unigram_grammar,
        )
    ...
    def fit_program_features(self):
        ...
        def replaceProgramsWithBigramLikelihoodSummaries(frontier):
            return Frontier(
                [
                    FrontierEntry(
                        program=self.bigram_grammar.closedLikelihoodSummary(
        ...
        self.frontier_likelihoods = {
            ...
            for (task, f) in self.frontiers.items()
            if not f.empty
        }

    def loss_bias_optimal(self, frontier_likelihood):
        batch_size = len(frontier_likelihood.entries)
        language_features = self.language_featurizer.featuresOfTask(
            frontier_likelihood.task
        )
        if language_features is None:
            return None

        language_features.detach()
        language_features = language_features.expand(
            batch_size, language_features.size(-1)
        )

        lls = self.log_linear_bigram_network.batchedLogLikelihoods(
            language_features, [entry.program for entry in frontier_likelihood]
        )
    ...
    def train(self, epochs=None, lr=0.001):
        ...
        eprint(
            "Fitting to {} frontiers with language data.".format(
                len(self.frontier_likelihoods)
            )
        )
    ...
    def enumerateFrontiers(
        ...
        with timing("Evaluated language-based transition model"):
            grammars = {task: self.contextual_grammar_for_task(task) for task in tasks}
