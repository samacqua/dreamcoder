...
class ClevrFeatureExtractor(RecurrentFeatureExtractor):
    ...
    serialize_special = (
        makeClevrTasks.serialize_clevr_object
    )  # Special flag passed to the OCaml Helmholtz enumerator.
    maximum_helmholtz = (
        5000  # Special flag passed to cap how many Helmholtz we enumerate.
    )
    ...
    def __init__(self, tasks, testingTasks=[], cuda=False):
        ...
        super(ClevrFeatureExtractor, self).__init__(
            lexicon=lexicon,
            H=64,
            tasks=tasks,
            bidirectional=True,
    ...
    def tokenize(self, task):
        ...
        sorted_tokenized = [
            e if example_len(e[0], e[1]) <= self.max_examples_length else blank_task()
            for e in sorted_tokenized
        ]
