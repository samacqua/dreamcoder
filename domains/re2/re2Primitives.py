...
from dreamcoder.domains.text.textPrimitives import (
    re2_text_primitives,
    re2_text_4_letter,
    re2_text_6_letter,
    re2_text_characters,
...
def _rmatch(s1):
    return lambda s2: __ismatch(s1, s2)


def _rsplit(s1):
    return lambda s2: __regex_split(s1, s2)


def _rflatten(l):
    return "".join(l)  # Flattens list of substrings back into a string


re2_rmatch = Primitive(
    "_rmatch", arrow(tsubstr, tsubstr, tbool), _rmatch, alternate_names=["regex_match"]
)
re2_rsplit = Primitive(
    "_rsplit",
...
def re2_primitives_main():
    ...
    if False:
        ...
        raw = (
            f"(lambda (_rflatten (map  {replace_te}  (_rsplit (_rconcat _t _e) $0) ) ))"
        )
        check_equal("replace te -> f", raw, "tehellote", "fhellof")
        # Prepend X to match
        prepend_te = (
            "(lambda  (if (_rmatch (_rconcat _t _e) $0) (_rconcat _f $0) $0)   )"
        )
        raw = (
            f"(lambda (_rflatten (map  {prepend_te}  (_rsplit (_rconcat _t _e) $0) ) ))"
        )
        check_equal("prepend te -> f", raw, "tehellote", "ftehellofte")
        # Postpend X to match
        postpend_te = (
            "(lambda  (if (_rmatch (_rconcat _t _e) $0) (_rconcat $0 _f) $0)   )"
        )
        ...
        raw = (
            f"(lambda (_rflatten (map  {remove_te }  (_rsplit (_rconcat _t _e) $0) ) ))"
        )
